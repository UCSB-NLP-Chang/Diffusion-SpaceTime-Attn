...
def greedy_Reg(
    ...
    ys_1 = encoder_output.new_full([batch_size, 1, 4], bos_index, dtype=torch.float)
    ...
    for step in range(max_output_length-1):
        step_template = template[:, :step+1].reshape(batch_size, step+1)
        # pylint: disable=unused-variable
        with torch.no_grad():
            output_cats, output_box = decoder(ys, ys_1, step_template,
            ...
            if step == 0:
                ps_1 = output_box.unsqueeze(1)
            elif step > 0:
                ps_1 = torch.cat([ps_1, output_box.unsqueeze(1)], dim=1)
            next_box = output_box.data
            ys_1 = torch.cat([ys_1, next_box.unsqueeze(1)], dim=1)

    return ps, ps_1, ys.detach(), ys_1.detach()
