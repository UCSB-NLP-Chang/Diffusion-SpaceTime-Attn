...
def greedy(
    ...
    ys_1 = encoder_output.new_full([batch_size, 1], bos_index, dtype=torch.long)
    ys_2 = encoder_output.new_full([batch_size, 1], bos_index, dtype=torch.long)
    ...
    for step in range(max_output_length-1):
        ...
        with torch.no_grad():
            output_cats, output_pos, output_shape = decoder(ys, ys_1, ys_2, encoder_output, src_mask, trg_mask)

            output_cats = output_cats[:, -1]
            if step == 0:
                ps = output_cats.unsqueeze(1)
            elif step > 0:
                ps = torch.cat([ps, output_cats.unsqueeze(1)], dim=1)
            _, next_word = torch.max(output_cats, dim=1)
            next_word = next_word.data
            ys = torch.cat([ys, next_word.unsqueeze(-1)], dim=1)

            output_pos = output_pos[:, -1]
            if step == 0:
                ps_1 = output_pos.unsqueeze(1)
            elif step > 0:
                ps_1 = torch.cat([ps_1, output_pos.unsqueeze(1)], dim=1)
            _, next_pos = torch.max(output_pos, dim=1)
            next_pos = next_pos.data
            ys_1 = torch.cat([ys_1, next_pos.unsqueeze(-1)], dim=1)

            output_shape = output_shape[:, -1]
            if step == 0:
                ps_2 = output_shape.unsqueeze(1)
            elif step > 0:
                ps_2 = torch.cat([ps_2, output_shape.unsqueeze(1)], dim=1)
            _, next_shape = torch.max(output_shape, dim=1)
            next_shape = next_shape.data
            ys_2 = torch.cat([ys_2, next_shape.unsqueeze(-1)], dim=1)
    ...
    return ps, ps_1, ps_2, ys.detach(), ys_1.detach(), ys_2.detach()
