...
with torch.no_grad():
    ...
    gt = "../../../scripts/output-combine-gpt3-test.txt"
    ...
    with open(gt, "r") as f:
        ...
        rows = contents.split('\n')
        for i in range(500):
            curr_relations = rows[4*i+1][10:].split(',')
            curr_relations_no_attributes = [] # [relation1, relation1-object1, relation1-object2, relation2, relation2-object1, relation2-object2]
            for each in curr_relations:
                # check which relation
                curr_relation = None
                for each_keyword in relation_keywords:
                    if each_keyword in each:
                        curr_relation = each_keyword
                assert curr_relation is not None
                curr_relations_no_attributes.append(curr_relation)
                # check what objects are involved, remove attributes
                involved_objects = each.split(curr_relation)
                for each_involved_object in involved_objects:
                    all_words = each_involved_object.split()
                    if len(all_words) > 1 and all_words[-2] + " " + all_words[-1] in ['hair drier', 'teddy bear', 'cell phone', 'dining table', 'potted plant', 'hot dog', 'wine glass', 'tennis racket', 'baseball glove', 'baseball bat', 'sports ball']:
                        curr_relations_no_attributes.append(all_words[-2] + " " + all_words[-1])
                    else:
                        curr_relations_no_attributes.append(all_words[-1])
