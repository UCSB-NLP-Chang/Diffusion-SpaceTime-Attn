...
mapping_objects = []
for each in mapping:
    mapping_objects.append(mapping[each])
...
with torch.no_grad():
    ...
    with open(gt, "rb") as f:
        ...
        for i in range(500):
            ...
            curr_objects = []
            for each_object in temp[i][-1]:
                for cand_object in mapping_objects:
                    if cand_object in each_object:
                        curr_objects.append(cand_object)
                        break
            curr_objects = sorted(set(curr_objects))
            gt_objects.append(curr_objects)
